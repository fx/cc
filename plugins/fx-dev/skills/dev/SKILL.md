...
**The materiality bar applies to judgment-originated findings only** — things a reviewer raised on its own reading, rather than violations of a rule the project wrote down (`references/scope-contract.md`). One of those must clear the bar before it can be treated as blocking. An observation that would change nothing if it shipped uncorrected is still recorded in the ledger, classified immaterial, and never triggers a rerun.
