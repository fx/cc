...
This skill can run **in parallel** with `fx-ultra:coderabbit-review` and any future automated-reviewer skills.

**Pick the execution mode based on your context (see `fx-ultra:dev` Step 6.3 for the full table):**
...
- **`fx-ultra:team` coordinator OR a sub-agent yourself** → sub-agents CANNOT spawn sub-agents. Run each reviewer's lifecycle sequentially yourself, optionally with the slow waiter (CodeRabbit) launched as a background `Bash` process while you handle Copilot in the foreground (mode B).
...
This skill expects a PR number. Pass it as args: `skill='fx-ultra:copilot-review', args='<PR_NUMBER>'`
...
```
Skill tool: skill="fx-ultra:resolve-pr-feedback", args="<PR_NUMBER>"
```
