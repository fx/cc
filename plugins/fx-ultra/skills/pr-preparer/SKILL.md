...
4. **Craft PR Description**: Create a **concise** PR description that includes ONLY:
   ...
   **PR Title Rules:**
   ...
   - **⛔ NEVER put `#<number>` in the title** (`#4`, `(#4)`, `#123`) unless N is a real, existing PR/issue in the **target repo** that this PR genuinely references. On squash-merge the title becomes the commit subject, where `#N` auto-links to PR/issue #N — so using it for an implementation **wave**, phase, step, or change-doc number wrongly cross-links the PR. This is BLOCKING. See the `fx-ultra:github` skill's "`#<number>` PR-Title Rule" for the full rule.
   ...
   - **Test plan** — a checklist of concrete verification steps someone (or the `fx-ultra:ultra-verifier` skill) can follow to prove the PR works end-to-end. Each item should be a checkbox:
     ```markdown
     ## Test plan
...
6. **Update Task Tracking**: Before creating the PR, check if relevant task tracking files exist. Search for:
   ...
   **MANDATORY: Load the project-management skill FIRST:**
   ```
   Skill tool: skill="fx-ultra:project-management"
   ```
