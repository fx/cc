---
name: team
description: "Spawn a coordinated sub-agent team to implement a spec or multi-task feature in parallel, under the fx-ultra rigor bar. Use when user says '/team', 'use a team', 'parallel implementation', 'work on multiple tasks', or provides a spec/issue that requires coordinated multi-PR work. The main session acts as coordinator — no code, no commits, only delegation and quality control — and enforces the ultra gates on every PR: 100% coverage, end-to-end ultra-verifier PASS, ultra-designer PASS for any UI, and a binding ultra-judge APPROVE (which can HALT)."
---
...
> - **Gate-check this before merge:** if a PR title contains a stray `#N` or wave/phase wording, rename it with `gh pr edit <N> --title "..."` before merging. A squash merge bakes the title into `main`'s history — a wrong cross-link there is permanent. See the `fx-ultra:github` skill's "`#<number>` PR-Title Rule".
...
**Load the dev skill** (`Skill tool: skill='fx-ultra:dev'`) and read its SDLC steps. The dev skill is the single source of truth for the development workflow — do not duplicate its instructions here.
...
**PR creation** → Either do it yourself via `gh pr create` or spawn a focused PR preparer agent. Load `fx-ultra:github` skill first. **⛔ If you create the PR yourself, the `--title` MUST be a conventional-commit subject — `type(scope): description` — matching the canonical regex `^(feat|fix|docs|refactor|chore|test|perf|build|ci|style|revert)(\(.+\))?!?: .+` (see the github skill's "Use Conventional Formats"). Do NOT write a prose title; running `gh pr create` directly does NOT exempt you from the conventional-commit rule. Verify the title against the regex before AND after creation.** (Prose titles the coordinator wrote directly — bypassing pr-preparer — are exactly how non-conventional titles have slipped onto `main`.)

**Review and CI steps** (Copilot review, CodeRabbit review, CI monitoring, feedback resolution) → **Handle these DIRECTLY as the coordinator.** These are lightweight skill/command invocations that must not be delegated. Invoke `fx-ultra:copilot-review` AND `fx-ultra:coderabbit-review` yourself (sequentially, or with the slow CodeRabbit waiter as a background Bash process while you handle Copilot in the foreground — see the Reviewer Gates section below). Run `gh pr checks --watch` yourself. Invoke `fx-ultra:resolve-pr-feedback` yourself.

**⛔ NEVER spawn sub-agents to handle reviewer waits.** `fx-ultra:dev` Step 6.3's mode A (parallel sub-agents per reviewer) is for the **root-session caller** only. As a team coordinator you ARE the root agent for the team's lifecycle and must use mode B (sequential or background-Bash overlap).

**Merge gates** → Always handle directly. See MANDATORY MERGE GATE CHECKLIST below.

**Ultra gates (verification, design, judge)** → For every PR, spawn the dedicated ultra teammates: `fx-ultra:ultra-verifier` (end-to-end, always), `fx-ultra:ultra-designer` (when there's a UI surface), and finally `fx-ultra:ultra-judge` (terminal, binding — can HALT). Also enforce 100% coverage on the diff. See the Ultra Gates section under the merge-gate checklist.
...
| 2 | **Copilot review RECEIVED and feedback RESOLVED** | Invoke `fx-ultra:copilot-review` skill — confirm 0 unresolved Copilot threads | YES |
| 2b | **CodeRabbit check terminal-passing AND feedback RESOLVED** | Invoke `fx-ultra:coderabbit-review` skill — confirm `CodeRabbit` check is `success` AND 0 unresolved CodeRabbit threads. CodeRabbit re-reviews on every push, so loop until convergence | YES (when configured) |
...
| 5b | **PR title is clean AND conventional** | Title (a) is a conventional-commit subject — run the canonical check from the `fx-ultra:github` skill's "Use Conventional Formats" (a plain prose title with no `type:` prefix FAILS) — AND (b) has NO stray `#<number>` (only a real PR/issue ref) and NO wave/phase/step/change-doc number. Fix with `gh pr edit <N> --title "type(scope): …"` before merge — squash bakes the title into `main` | YES |
| 6 | **100% coverage on changed code** | Coverage report shows 100% line+branch on the diff (ULTRA MANDATE #2, `fx-ultra:dev` STEP 4.6) — not lowered, not excluded, not faked | YES |
| 7 | **Ultra-Verification PASS** | `fx-ultra:ultra-verifier` returned PASS for this PR (≥2 passes, every observable item proven against the real running stack with direct evidence). See Ultra Gates below | YES |
| 8 | **Ultra-Design PASS (if any visual surface)** | `fx-ultra:ultra-designer` returned PASS (full interactivity-state matrix + responsive + a11y), OR the PR has no visual surface. See Ultra Gates below | YES (when UI) |
| 9 | **Ultra-Judge APPROVE** | `fx-ultra:ultra-judge` audited the whole PR progression and returned **APPROVE** (not REMEDIATE, not HALT). A HALT freezes this PR and escalates to the user. See Ultra Gates below | YES |

### ⛔ Reviewer Gates (Gates 2 + 2b) — CRITICAL

> **CodeRabbit and Codex run LOCALLY first.** Implementing sub-agents MUST run a local CodeRabbit review via the `cr` CLI AND a local Codex review via `codex review --base main` during pre-PR self-review (`fx-ultra:dev` Step 4.5 / `fx-ultra:coderabbit-review` Mode 1 / `fx-ultra:codex-review`) and open the PR only once both are clean. Gate 2b below is the **fallback** PR-level CodeRabbit gate — it applies only when the repo's CodeRabbit GitHub App also auto-reviews PRs. If a clean local `cr` review ran and no `CodeRabbit` check appears, Gate 2b is already satisfied.

**As coordinator, YOU handle reviewer waits directly. Do NOT spawn sub-agents for reviewer waits — sub-agents in this team context cannot spawn their own sub-agents, and `fx-ultra:dev` mode A would fail. You ARE the root agent for the team; invoke each reviewer skill in the foreground sequentially, OR launch the slow waiter (CodeRabbit) as a background `Bash` process while you handle Copilot in the foreground.**

```
# Sequential (simple, always correct):
Skill tool: skill="fx-ultra:copilot-review",     args="<PR_NUMBER>"
Skill tool: skill="fx-ultra:coderabbit-review",  args="<PR_NUMBER>"
...
# 2. Foreground: Skill fx-ultra:copilot-review
# 3. When Bash task completes: Skill fx-ultra:rabbit-feedback-resolver
```
...
### ⛔ Ultra Gates (Gates 6–9) — THE WHOLE POINT OF fx-ultra

These are the gates that make this `fx-ultra`, not `fx-dev`. They are defined authoritatively in the `fx-ultra:dev` source of truth (STEP 4.6, 5.5, 5.6, 9); the coordinator enforces them per PR. **None may be skipped, and PR size is never an excuse.**

#### Gate 6 — 100% coverage on the diff
Before merge, confirm the PR achieves **100% line+branch coverage on changed code**. The implementing coder is responsible for it (`fx-ultra:dev` STEP 4.6); as coordinator, verify it from the real coverage report / Codecov, and reject any PR that reaches the number by lowering thresholds, excluding the changed files, or skipping tests. If coverage is short, send the coder back (or spawn a focused test agent) before merging.

#### Gate 7 — Ultra-Verification (end-to-end, PROVE it works)
Spawn a dedicated verification teammate that loads `fx-ultra:ultra-verifier` (this supersedes the old browser-only verify agent — it covers web, mobile, CLI, API/backend, desktop, libraries):

```
Agent tool:
  name: "ultra-verify-<pr-number>"      # REQUIRED — addressable handle (do NOT pass team_name; it's ignored)
  prompt: "Load the ultra-verifier skill (Skill tool: skill='fx-ultra:ultra-verifier').
           Verify PR #<NUMBER> on branch <branch-name> END-TO-END against the REAL running stack.
           - Detect & launch the repo's actual local stack; drive every changed observable
             behavior by the correct modality (web→Playwright MCP, mobile→emulator, CLI→real
             invocation, API→real requests + DB inspection, etc.).
           - Capture direct-observation evidence (DOM/a11y snapshots, console, network, API/DB,
             stdout/exit codes) — NEVER screenshots, NEVER 'looks right'.
           - Run the full verification ≥2×; a 3rd pass if anything is ambiguous/flaky.
           - Emit the strict ULTRA-VERIFIER VERDICT block (PASS/FAIL/BLOCKED) with per-item evidence."
  description: "Ultra-verify PR #<NUMBER> end-to-end"
  mode: "bypassPermissions"
```

Require a **PASS** verdict. On FAIL, send a coder to fix and re-spawn the verifier (max 3 iterations). A **BLOCKED** is NOT a pass — report the precise blocker to the user and fall back to user-confirmed manual verification for the affected items.

**Why this gate exists:** CI does NOT catch runtime-only errors (circular deps, SSR failures, broken module init) or behavior regressions. ultra-verifier proves the change actually works in a running system.

#### Gate 8 — Ultra-Design (visual/UX polish, when there's a UI surface)
If the PR touches anything a user sees, spawn an ultra-designer teammate (it inspects the running app's live DOM/computed styles — never screenshots):

```
Agent tool:
  name: "ultra-design-<pr-number>"      # REQUIRED
  prompt: "Load the ultra-designer skill (Skill tool: skill='fx-ultra:ultra-designer').
           Review the visual/UX surfaces changed in PR #<NUMBER> (branch <branch-name>) with
           extreme prejudice against the skill's design principles: hierarchy/spacing/type/
           contrast/alignment, the FULL interactivity-state matrix (default/hover/focus-visible/
           active/disabled/loading/selected/error/empty), motion where warranted + reduced-motion,
           responsive sweep across viewports, and accessibility. Evidence = computed styles /
           a11y-tree facts via Playwright MCP, NEVER screenshots.
           Emit the strict ULTRA-DESIGNER VERDICT block (PASS/FAIL/BLOCKED) with per-finding evidence."
  description: "Ultra-design review PR #<NUMBER>"
  mode: "bypassPermissions"
```

Require **PASS**. On FAIL, send a coder to fix and re-spawn the designer (max 3 iterations). Do NOT merge un-polished UI.

#### Gate 9 — Ultra-Judge (terminal, BINDING, can HALT)
**LAST gate before merge for every PR.** Spawn an ultra-judge teammate to audit the entire PR progression from primary evidence:

```
Agent tool:
  name: "ultra-judge-<pr-number>"       # REQUIRED
  prompt: "Load the ultra-judge skill (Skill tool: skill='fx-ultra:ultra-judge').
           Render a final, adversarial verdict on PR #<NUMBER> (branch <branch-name>) from PRIMARY
           evidence (diff, real coverage report, ultra-verifier & ultra-designer verdict blocks,
           CI status, review-thread state). Scrutinize verification, polish, and 100% coverage for
           shortcuts/fakes. Emit the strict ULTRA-JUDGE VERDICT block (APPROVE/REMEDIATE/HALT) with
           per-item evidence and, if not APPROVE, a numbered remediation list."
  description: "Ultra-judge PR #<NUMBER>"
  mode: "bypassPermissions"
```

**The verdict is BINDING on you, the coordinator:**
- **APPROVE** → proceed to merge (after all other gates).
- **REMEDIATE** → execute the numbered remediation (spawn fix agents / re-run the relevant gate), then **re-spawn ultra-judge from scratch**. Do NOT merge.
- **HALT** → **FREEZE this PR immediately.** Do NOT merge it. Do NOT spawn dependent work that builds on it. Surface the judge's HALT block verbatim to the user with the exact violation, and wait for user direction. A HALT means rigor was bypassed (verification/coverage/polish faked or skipped) or a prior REMEDIATE was ignored — **merging past a HALT is the single worst thing this team can do.** If you ever catch yourself rationalizing past a HALT to "keep the waves moving," stop and escalate instead.

**If a "small" or "follow-up" PR:** Same rules. No exceptions. PR size is NEVER a reason to skip the ultra gates.
...
1. Verify all spec tasks are marked done (load `fx-ultra:project-management` to check)
...
- **NEVER merge without Copilot review** — always invoke `fx-ultra:copilot-review` yourself. No exceptions.
- **NEVER merge without CodeRabbit review when CodeRabbit is configured** — always invoke `fx-ultra:coderabbit-review` yourself; cycle until its check is `success` and 0 threads. No exceptions.
...
- **ALWAYS use `fx-ultra:project-management`** to verify task tracking
- **ALWAYS run the full merge gate checklist** even for "trivial" or "follow-up" PRs
- **NEVER merge without the ULTRA GATES (6–9)** — 100% diff coverage, an `fx-ultra:ultra-verifier` PASS, an `fx-ultra:ultra-designer` PASS for any UI surface, and an `fx-ultra:ultra-judge` APPROVE. CI alone does NOT catch runtime errors, missing polish, or faked rigor.
- **NEVER merge or build dependent work past an `fx-ultra:ultra-judge` HALT** — a HALT freezes the PR and escalates to the user verbatim. Rationalizing past a HALT to keep waves moving is the single worst thing this team can do.
- **NEVER lower the bar to what the spec asks for** — 100% coverage and full end-to-end verification are mandatory even when the task description asks for less.
