...
|----------|---------------|----------------|
| GitHub Copilot | `Copilot` | `fx-ultra:copilot-feedback-resolver` |
| CodeRabbit | `coderabbitai[bot]` | `fx-ultra:rabbit-feedback-resolver` |
| Codecov | `codecov[bot]` / `codecov-commenter` | `fx-ultra:resolve-codecov-feedback` |

## Prerequisites

**CRITICAL: Load the `fx-ultra:github` skill FIRST** before running any GitHub API operations.
...
```
Skill tool: skill="fx-ultra:copilot-feedback-resolver"
```

**If CodeRabbit threads exist:**
```
Skill tool: skill="fx-ultra:rabbit-feedback-resolver"
```

**If Codecov coverage gaps detected:**
```
Skill tool: skill="fx-ultra:resolve-codecov-feedback"
```

**If multiple exist:** Prefer running Copilot and CodeRabbit resolvers **in parallel** by spawning each as a sub-agent in the same message (see `fx-ultra:dev` Step 6.3 for the exact pattern). Codecov is sequential after them since coverage fixes typically require code from the other resolvers to be in place first.
...
1. Wait for all reviewer checks to reach terminal state (use the dedicated waiters: `fx-ultra:copilot-review` for Copilot, `fx-ultra:coderabbit-review` for CodeRabbit).
...
### Resolution
- Invoked fx-ultra:copilot-feedback-resolver
- Invoked fx-ultra:rabbit-feedback-resolver
- Invoked fx-ultra:resolve-codecov-feedback
